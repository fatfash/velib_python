	...
	def flush(self):
		if self.changes:
			self.parent._dbusnodes['/'].PropertiesChanged({
				'Value': {
					path: change['Value'] for path, change in self.changes.iteritems()
				},
				'Text': {
					path: change['Text'] for path, change in self.changes.iteritems()
				}
			})
...
class VeDbusRootTracker(object):
	...
	def _root_properties_changed_handler(self, changes):
		...
		texts = changes['Text']
		for p, v in changes['Value'].iteritems():
			try:
				t = texts[p]
			except KeyError:
...
class VeDbusItemImport(object):
	def __new__(cls, bus, serviceName, path, eventCallback=None, createsignal=True):
		instance = object.__new__(cls, bus, serviceName, path, eventCallback=eventCallback, createsignal=createsignal)
